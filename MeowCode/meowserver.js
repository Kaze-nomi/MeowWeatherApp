...
app.post('/proxy/gpt', async (req, res) => {
    ...
    try {
        ...
        Предложи, чем можно заняться, учитывая эти условия. Ответ должен быть кратким (3-4 предложения) и не содержать обоснований или извинений. В твоём ответе ты не в коем случае не должен говорить, какая сейчас температура, пользователь и без твоего сообщения знает её. Желательно, чтобы ты упомянул особенности города или места откуда идёт запрос. Учитывай дату и время в текущем регионе.`
                }
            ],
            "stream": false,
            "update_interval": 0
